...
describe("CheckoutFlow", () => {
  ...
  test("should render categories", async () => {
    ...
    await waitFor(() => {
      expect(screen.getAllByTestId("category-tabs")[0]).toBeInTheDocument();
    });
  });
  ...
  test("should render categories items", async () => {
    ...
    await waitFor(() => {
      expect(screen.getAllByTestId("category-items")[0]).toBeInTheDocument();
    });
  });
  ...
  test("should not process the order", async () => {
    ...
    textboxs.map((textbox) => {
      userEvent.type(textbox, "4004 40004 4004 40004");
    });
